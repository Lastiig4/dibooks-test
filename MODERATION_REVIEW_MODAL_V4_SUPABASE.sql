-- DiBooks - Moderation Alert Navigator V3
-- Uitvoeren NA V2. Mag veilig opnieuw worden uitgevoerd.
--
-- V3:
-- - per waarschuwing geen bevestigingspopup/motivatie meer
-- - admin kan een afgehandelde waarschuwing heropenen
-- - bij uiteindelijke GOEDKEURING van een boek met high-flags is
--   één algemene motivatie verplicht
-- - alle open flags moeten nog steeds eerst afgehandeld zijn
...
create or replace function public.reopen_moderation_flag(
  input_flag_id uuid
)
returns boolean
language plpgsql
...
begin
  if not public.is_current_dibooks_admin() then
    raise exception 'Alleen admins kunnen een moderatiemelding heropenen.';
  end if;
...
revoke all on function public.reopen_moderation_flag(uuid) from public;
grant execute on function public.reopen_moderation_flag(uuid) to authenticated;


-- Review-detail inclusief persisted flagstatus.
...
-- Finale review:
-- 1. alle flags moeten cleared zijn voordat approval kan
-- 2. als er minimaal één HIGH flag bestaat, moet de admin één algemene
--    approval-motivatie meesturen
...
begin
  ...
  if input_decision = 'approved' then
    ...
    if high_flag_count > 0
       and length(btrim(coalesce(input_feedback,''))) = 0 then
      raise exception 'Bij goedkeuring van een boek met ernstige meldingen is één algemene motivatie verplicht.';
    end if;
  end if;
  ...
  if input_decision = 'approved' then
    ...
    set status = 'approved',
        reviewed_at = now(),
        reviewed_by = reviewer_id,
        review_feedback = nullif(btrim(coalesce(input_feedback,'')), '')
...
-- ============================================================
-- V4: toon de admin die een boek heeft beoordeeld in de queuehistorie.
-- Return type verandert, dus de functie moet eerst worden gedropt.
-- ============================================================
