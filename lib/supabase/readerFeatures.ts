...
export type ReadingStoryValue = boolean | number | string;
export type ReadingStoryState = Record<string, ReadingStoryValue>;
...
export function canUserReadBookAccess(
...
export function getReadBlockReason(
  ...
  if (
    normalizeAccessType(accessType) === "premium" &&
    !canReadPremiumBooks(user)
  ) {
...
function mapFavoriteRow(row: any): FavoriteBook {
  return {
    ...
    coverClass:
      row.cover_class ?? "from-blue-950 via-slate-950 to-purple-950",
    ...
    progressPageIndex:
      typeof row.progress_page_index === "number"
        ? row.progress_page_index
        : undefined,
    progressPercent:
...
export async function isBookFavorite(
  user: DemoAuthUser | null,
  bookId: string,
) {
...
export async function getReadingProgress(
  ...
  const { data, error } = await supabase
    ...
    .select(
      "book_id,current_node_id,page_index,progress_percent,story_state,updated_at",
    )
    .eq("user_id", user.id)
    .eq("book_id", bookId)
    .maybeSingle();
  ...
  const storyState =
    data.story_state &&
    typeof data.story_state === "object" &&
    !Array.isArray(data.story_state)
      ? (data.story_state as ReadingStoryState)
      : {};
...
export async function upsertReadingProgress(
  ...
  // Optioneel houden voorkomt dat een eventuele oudere caller per ongeluk
  // bestaande verhaalstatus wist door deze parameter niet mee te geven.
  if (storyState !== undefined) {
    payload.story_state = storyState;
  }
...
export async function fetchReadingProgressBooks(
  user: DemoAuthUser | null,
) {
  if (!user) return [] as FavoriteBook[];
